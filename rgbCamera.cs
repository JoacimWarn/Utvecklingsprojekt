...
namespace ConsoleApp3
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Mat image;
            Mat[] cameraChannels = new Mat[3];

            while (true)
            {

                image = program.GrabFrame();

                Cv2.Split(image, out cameraChannels);

                Cv2.ImShow("blueFrame", cameraChannels[0]);
                Cv2.ImShow("greenFrame", cameraChannels[1]);
                Cv2.ImShow("redFrame", cameraChannels[2]);
